Vue.component('by-report', {
    template: '#stat_by_report_tpl',
    props: ['currency_id'],
    mixins: [reports_mixin],
    data: () => {
        return {
            traffic: StatColumns.columns.report.traffic,
            coefficient: StatColumns.columns.report.coefficient,
            conversion: StatColumns.columns.report.conversion,
            finance: StatColumns.columns.report.finance,
            table_id: 'report-tables table',
        }
    },

    created() {
        bus.$on('stat-settings-updated', stat_settings => {
            this.traffic = stat_settings.report.traffic;
            this.coefficient = stat_settings.report.coefficient;
            this.conversion = stat_settings.report.conversion;
            this.finance = stat_settings.report.finance;
        });

        filters_bus.$on('grouping-fields-init', params => {
    ...
    methods: {
        getStat() {
            ...
            api.get('/stat.getReport', {params: params}).then(response => {
