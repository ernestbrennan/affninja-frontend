...
<ul class="list-group aside-list-group">
  <li @click="categoryUpdated(0)"
      :class="['list-group-item pointer list-header', category_id === 0 ? 'active' : '']">
    {{ trans('filters.all_offer_categories') }}
  </li>
  <li v-for="category in categories" @click="categoryUpdated(category.id)"
      :class="['list-group-item pointer nowrap', category.id === category_id ? 'active' : '']">
    @{{ category.title }}
